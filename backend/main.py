...
def init_db():
    ...
    try:
        ...
        # social_channel_events table is created automatically by create_all,
        # but if it was added after the first run we need to handle it gracefully.
        existing_tables = insp.get_table_names()
        if "social_channel_events" not in existing_tables:
            from app.models.models import SocialChannelEvent  # noqa: F401
            Base.metadata.tables["social_channel_events"].create(bind=engine, checkfirst=True)
