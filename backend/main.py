    ...
    try:
        ...
        if "project_id" not in order_cols:
            with engine.connect() as conn:
                ...
                conn.commit()
        # Dedup indexes for lead source URLs
        with engine.connect() as conn:
            conn.execute(
                text(
                    "CREATE UNIQUE INDEX IF NOT EXISTS uq_leads_source_website_nonempty "
                    "ON leads (source, website) WHERE source <> '' AND website <> ''"
                )
            )
            conn.execute(
                text(
                    "CREATE UNIQUE INDEX IF NOT EXISTS uq_b2b_leads_source_url_nonempty "
                    "ON b2b_leads (source, url) WHERE source <> '' AND url <> ''"
                )
            )
            conn.commit()
...
async def lifespan(application):
    ...
    b2b_scanner.start_b2b_scheduler()
    yield
