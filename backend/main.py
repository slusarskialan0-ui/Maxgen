...
def init_db():
    ...
    try:
        ...
        if db.query(Automation).count() == 0:
            ...
            db.add(Automation(name="Auto-oferta dla gorącego leada", trigger="score_above", action="offer", condition_json='{"threshold": 60}', enabled=True))
            db.add(Automation(name="Auto-zamknięcie premium", trigger="score_above", action="close", condition_json='{"threshold": 90}', enabled=True))
