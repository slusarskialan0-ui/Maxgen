...
def init_db():
    ...
    try:
        ...
        if db.query(Automation).count() == 0:
            db.add(Automation(name="Auto-przypisz nowego leada", trigger="new_lead", action="assign", enabled=True))
            db.add(Automation(name="Auto-follow-up po 24h", trigger="new_lead", action="follow_up", enabled=True))
            db.add(Automation(name="Priorytetyzuj wysokie score", trigger="score_above", action="assign", condition_json='{"threshold": 70}', enabled=True))
            db.add(Automation(name="Auto-zamknięcie niskiej jakości", trigger="score_below", action="close", condition_json='{"threshold": 20}', enabled=True))
            db.add(Automation(name="Auto-oferta dla gorących leadów", trigger="score_above", action="offer", condition_json='{"threshold": 80}', enabled=True))
            db.add(Automation(name="Auto-predykcja konwersji", trigger="new_lead", action="predict", enabled=True))
            db.commit()
