...
from app.models.models import Industry, VoivodeshipStatus, SystemConfig
...
from app.routers import telegram_bot, setup as setup_router, health as health_router, b2b_scanner
...
async def lifespan(application):
    init_db()
    health_router.start_cron_sweeper()
    yield
...
# B2B Lead Engine routers
app.include_router(telegram_bot.router)
app.include_router(setup_router.router)
app.include_router(health_router.router)
app.include_router(b2b_scanner.router)
...
class SaveStateRequest(BaseModel):
    key: str = Field(min_length=1, max_length=190)
    value: Any


def _save_state_to_db(key: str, value: Any) -> None:
    db: Session = SessionLocal()
    try:
...
def _read_state_from_db(key: str) -> Any:
    db: Session = SessionLocal()
    try:
        row = db.query(SystemConfig).filter(SystemConfig.key == key).first()
        if not row:
...
@app.post("/api/save", tags=["state"])
def api_save(body: SaveStateRequest):
    _save_state_to_db(body.key, body.value)
    return {"status": "ok", "key": body.key}


@app.get("/api/get-data", tags=["state"])
def api_get_data(key: str):
    return {"status": "ok", "key": key, "value": _read_state_from_db(key)}


@app.get("/api/niche-status", tags=["state"])
def api_niche_status(niche: str | None = None):
    db: Session = SessionLocal()
    try:
        if niche:
            safe_niche = niche.strip()
            if not safe_niche or len(safe_niche) > 120:
                raise HTTPException(status_code=422, detail="Invalid niche parameter")
            row = db.query(SystemConfig).filter(SystemConfig.key == f"niche_status:{safe_niche}").first()
            if not row:
                return {"status": "ok", "niche": safe_niche, "data": None}
            try:
                parsed = json.loads(row.value)
            except Exception:
                parsed = row.value
            return {"status": "ok", "niche": safe_niche, "data": parsed, "updated_at": row.updated_at}

        rows = (
            db.query(SystemConfig)
            .filter(SystemConfig.key.like("niche_status:%"))
            .order_by(SystemConfig.updated_at.desc())
            .all()
        )
        items = []
        for row in rows:
            niche_name = row.key.split("niche_status:", 1)[-1]
            try:
                parsed = json.loads(row.value)
            except Exception:
                parsed = row.value
            items.append({"niche": niche_name, "data": parsed, "updated_at": row.updated_at})
