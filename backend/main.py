...
router_modules = _load_router_modules()
security = router_modules.get("security")
devplatform = router_modules.get("devplatform")
if security is None or devplatform is None:
    raise RuntimeError("Required router modules missing: security/devplatform")
...
for _, module in sorted(router_modules.items(), key=lambda item: item[0]):
