...
class ApiSystemTests(unittest.TestCase):
    ...
    def test_cloud_state_bridge_endpoints(self):
        save = self.client.post(
            "/api/save",
            json={"key": "auto_leads_state", "value": {"selectedVoivodeship": "mazowieckie", "selectedIndustries": ["mechanik"]}},
        )
        self.assertEqual(save.status_code, 200)
        self.assertEqual(save.json()["status"], "ok")

        data = self.client.get("/api/get-data", params={"key": "auto_leads_state"})
        self.assertEqual(data.status_code, 200)
        self.assertEqual(data.json()["value"]["selectedVoivodeship"], "mazowieckie")

        save_niche = self.client.post(
            "/api/save",
            json={"key": "niche_status:mazowieckie", "value": {"status": "zakonczone"}},
        )
        self.assertEqual(save_niche.status_code, 200)

        niche = self.client.get("/api/niche-status", params={"niche": "mazowieckie"})
        self.assertEqual(niche.status_code, 200)
        self.assertEqual(niche.json()["data"]["status"], "zakonczone")
