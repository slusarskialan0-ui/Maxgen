...
from config import build_db_connect_args, normalize_database_url  # noqa: E402
...
class ApiSystemTests(unittest.TestCase):
    ...
    def test_required_tables_exist(self):
        ...
        required = {"leads", "subscribers", "users", "campaigns", "offers", "payments", "automations", "audit_logs"}
        self.assertTrue(required.issubset(tables))

    def test_neon_database_url_is_normalized(self):
        raw_url = "postgresql://example.com/leads%20?sslmode=require&amp;channel_binding=require"
        normalized = normalize_database_url(raw_url)
        self.assertTrue(normalized.startswith("postgresql://"))
        self.assertEqual(
            normalized,
            "postgresql://example.com/leads?sslmode=require&channel_binding=require",
        )
        self.assertEqual(build_db_connect_args(normalized), {})

    def test_neon_database_url_adds_required_ssl_params(self):
        normalized = normalize_database_url("postgresql://example.com/leads")
        self.assertIn("sslmode=require", normalized)
        self.assertIn("channel_binding=require", normalized)

    def test_postgres_scheme_is_upgraded(self):
        normalized = normalize_database_url("postgres://example.com/leads")
        self.assertTrue(normalized.startswith("postgresql://"))
