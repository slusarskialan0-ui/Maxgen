...
class IntegrationJob(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    connector = Column(String, index=True, nullable=False)
    direction = Column(String, default="export")  # export, import, sync
    status = Column(String, default="pending")  # pending, processing, done, failed, dead_letter
