...
from app.models.models import Lead, BackupRecord, Campaign
...
def create_backup(db: Session = Depends(get_db)):
    return _create_backup(db, auto=False)


def _create_backup(db: Session, auto: bool = False):
    ...
    db.add(rec)
    db.commit()
    return {"ok": True, "filename": filename, "size_bytes": size, "auto": auto}
...
@router.post("/clean")
def clean_lost_leads(db: Session = Depends(get_db)):
...
def auto_backup(db: Session = Depends(get_db)):
    return _create_backup(db, auto=True)
...
def sync_status(db: Session = Depends(get_db)):
    ...
    return {
        "last_backup": latest.created_at.isoformat() if latest and latest.created_at else None,
        "last_backup_filename": latest.filename if latest else None,
