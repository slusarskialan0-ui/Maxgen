...
api_keys = {
    str(uuid.uuid4()): {"status": "active", "created_at": datetime.now(timezone.utc).isoformat()}
}
api_request_log = []
...
def rotate_keys():
    ...
    api_keys[new_key] = {"status": "active", "created_at": datetime.now(timezone.utc).isoformat()}
