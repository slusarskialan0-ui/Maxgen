...
def _rules_query(db: Session):
    return db.query(Automation).filter(or_(Automation.automation_type.is_(None), Automation.automation_type == ""))


@router.get("")
def list_automations(db: Session = Depends(get_db)):
    rows = _rules_query(db).order_by(Automation.id).all()
