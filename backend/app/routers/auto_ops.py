...
def run_sales_automation(db: Session = Depends(get_db)):
    ...
    for lead in leads:
        ...
        if lead.id not in existing_offer_lead_ids:
            ...
            db.add(Offer(
                project_id=lead.project_id or "default",
                lead_id=lead.id,
...
def simulate_payments(db: Session = Depends(get_db)):
    ...
    for idx, offer in enumerate(offers, start=1):
        ...
        payment = Payment(
            project_id=offer.project_id or "default",
            lead_id=offer.lead_id,
            offer_id=offer.id,
            amount=float(offer.amount or offer.price or 0),
            status=status,
...
def revenue_dashboard(db: Session = Depends(get_db)):
    ...
    pipeline_value = round(sum(float(offer.amount or offer.price or 0) for offer in db.query(Offer).all()), 2)
    return {
...
def list_automations(db: Session = Depends(get_db)):
    rows = (
        db.query(Automation)
        .filter(Automation.automation_type.isnot(None), Automation.automation_type != "")
        .order_by(Automation.automation_type.asc())
        .all()
    )
