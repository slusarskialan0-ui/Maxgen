...
from app.models.models import Lead, Campaign, Automation, UserProfile, Offer, LogEntry
...
class OfflineGeneratePayload(BaseModel):
    count: int = 40
    project_id: str = "default"
    campaign_id: Optional[int] = None
        ...
        for auto in automations:
            ...
            if auto.trigger == "new_lead":
                ...
                try:
                    ...
                    pass
            elif auto.trigger == "score_below":
                try:
                    cond = json.loads(auto.condition_json)
                    triggered = lead.score <= float(cond.get("threshold", 20))
                except Exception:
                    pass
            if triggered:
                if auto.action == "assign" and not lead.assigned_to:
                    ...
                    exists = db.query(Offer).filter(Offer.lead_id == lead.id, Offer.status.in_(["draft", "sent"])).first()
                    if not exists:
                        amount = round(max(500.0, lead.score * 85), 2)
                        db.add(Offer(
                            lead_id=lead.id,
                            campaign_id=lead.campaign_id,
                            title=f"Oferta dla {lead.company or lead.full_name or f'Lead #{lead.id}'}",
                            description="Automatycznie wygenerowana oferta offline.",
                            amount=amount,
                            currency="PLN",
                            status="sent",
                            valid_until=datetime.utcnow() + timedelta(days=14),
                        ))
                        db.add(LogEntry(module="automations", level="INFO", event="offer_created", details=f"lead_id:{lead.id} amount:{amount}"))
...
def generate_offline_leads(payload: OfflineGeneratePayload, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    sources = [
        ("reklamy_offline", "kampania_lokalna"),
        ("social_offline", "post_spolecznosciowy"),
        ("marketplace_offline", "oferta_rynkowa"),
        ("formularze_offline", "formularz_papierowy"),
    ]
    industries = ["Moto", "Transport", "Detailing", "Serwis", "Leasing", "Ubezpieczenia"]
    voivodeships = ["Mazowieckie", "Małopolskie", "Śląskie", "Pomorskie", "Dolnośląskie", "Wielkopolskie"]
    behaviors = ["szybki_kontakt", "porownuje_oferty", "zainteresowany_finansowaniem", "pilny_zakup", "niskoaktywny"]
    created = []
    total = max(1, min(payload.count, 500))
    for idx in range(total):
        source, detail = sources[idx % len(sources)]
        lead = Lead(
            full_name=fake.name(),
            email=fake.company_email() if random.random() > 0.25 else "",
            ...
            assigned_to="",
            stage="nowy",
            notes=f"offline_source:{detail}; zachowanie:{random.choice(behaviors)}",
            tags="offline,generator",
            project_id=payload.project_id,
        )
        lead.assigned_to = _auto_assign(db)
        lead.score = _compute_score(lead)
        lead.conversion_probability = _compute_conversion(lead.score, lead.stage)
        lead.follow_up_at = datetime.utcnow() + timedelta(hours=random.randint(12, 72))
        db.add(lead)
        db.flush()
        created.append(lead.id)
    db.add(LogEntry(module="leads", level="INFO", event="offline_leads_generated", details=f"count:{len(created)}"))
    db.commit()
    for lead_id in created:
        background_tasks.add_task(_run_automations, lead_id)
    return {"ok": True, "created": len(created), "lead_ids": created}
