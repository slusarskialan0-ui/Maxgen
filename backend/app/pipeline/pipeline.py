...
from app.models.models import Client, Order, AcquisitionLog, VoivodeshipStatus, Location, Automation
from app.data.geography import ORDER_TEMPLATES
from app.pipeline.recovery import record_source_success, record_source_failure
...
def _fetch_source(source: BaseSource, voivodeship: str, industries: list[str]):
    try:
        ...
        record_source_failure(source.source_type, str(exc))
...
def run_pipeline(voivodeship: str, industries: list[str], db: Session) -> dict:
    ...
    # Trigger automations for new_lead event
