...
1. Utwórz aplikację na [Fly.io](https://fly.io)
2. Użyj konfiguracji z `fly.toml` i wdroż backend (`flyctl deploy`)
3. Ustaw zmienne środowiskowe:
   ```
   DATABASE_URL=postgresql://...   # opcjonalnie Postgres
   CORS_ORIGINS=https://twoj-frontend.vercel.app
   PORT=8000
   ```
4. URL backendu: `https://polska-auto-leads-engine-max-b1fmoq.fly.dev`
5. Jeśli zmienisz nazwę aplikacji Fly.io lub domenę backendu, zaktualizuj cel proxy w `frontend/vercel.json` (`/api/*`).
...
3. `vercel.json` automatycznie proxyfikuje `/api/*` do backendu na Fly.io
4. `VITE_API_URL` jest opcjonalne (użyj tylko jeśli chcesz nadpisać domyślny backend)
...
| Zmienna | Opis | Domyślnie |
|---------|------|-----------|
| `VITE_API_URL` | Opcjonalny override URL backendu | `http://localhost:8000` |
