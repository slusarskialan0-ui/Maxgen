...
2. Z repo uruchom `fly launch --no-deploy`, a następnie dopasuj nazwę aplikacji w `fly.toml`
3. Wdróż backend komendą `fly deploy` albo przez workflow `.github/workflows/fly-backend-deploy.yml`
4. Ustaw sekrety / zmienne środowiskowe:
   ```
   DATABASE_URL=postgresql://...   # zewnętrzny Postgres
   CORS_ORIGINS=https://twoj-frontend.vercel.app
   PORT=8000
   ```
5. Skopiuj URL backendu (np. `https://twoj-backend.fly.dev`)
...
3. Ustaw zmienne środowiskowe w Vercel:
   ```
   VITE_API_URL=https://twoj-backend.fly.dev
   ```
...
├── backend/                    # FastAPI backend (Fly.io / Docker)
...
├── Procfile                    # Legacy Railway entry point
...
| Zmienna | Opis | Domyślnie |
|---------|------|-----------|
| `VITE_API_URL` | Backend URL (Fly.io) | `http://localhost:8000` |
