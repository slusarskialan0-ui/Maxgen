...
| `RATE_LIMIT_REQUESTS` | Limit requestów na IP | `100` |
| `RATE_LIMIT_WINDOW_SECONDS` | Okno limitu (sekundy) | `60` |
...
### GitHub Actions — auto deploy po pushu
Workflow `.github/workflows/railway-vercel-deploy.yml` uruchamia deploy automatycznie (main/master), jeśli ustawisz sekrety:
- Railway: `RAILWAY_TOKEN`, `RAILWAY_SERVICE`, opcjonalnie `RAILWAY_PROJECT_ID`
...
## ✅ Smoke test API (szybka weryfikacja)
...
curl -X POST http://localhost:8000/pipeline/run \
  ...
  -d '{"voivodeship":"mazowieckie"}'
```

`POST /pipeline/run` wymaga body JSON. Wywołanie bez body zwróci `422 Unprocessable Entity`.
