...
4. Ustaw zmienne środowiskowe w `.env` / panelu deploy:
   ```
   DATABASE_URL=******HOST/DB_NAME?sslmode=require&channel_binding=require
...
|---------|------|-----------|
| `DATABASE_URL` | PostgreSQL / Neon URL z SSL | `sqlite:///./polska_leads.db` (fallback, override in `.env`) |
