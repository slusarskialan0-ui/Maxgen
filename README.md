...
POST /leads/generate-offline   → generator leadów offline
GET  /offers                   → lista ofert
POST /offers                   → utworzenie oferty
GET  /payments                 → lista płatności
POST /payments                 → utworzenie płatności (offline)
POST /payments/{id}/confirm    → potwierdzenie płatności
GET  /payments/logs/recent     → logi płatności offline
