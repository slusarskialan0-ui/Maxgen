...
## Tryb „na stałe” (always-on)

- Frontend cyklicznie sprawdza `GET /health` backendu.
- Po utracie łączności i powrocie internetu aplikacja wywołuje sync przez Service Worker.
- Service Worker przetwarza kolejkę offline po odzyskaniu połączenia.
- Aktualizacje PWA instalują się automatycznie po publikacji nowej wersji.
- Endpointy `/system/liveness` i `/system/readiness` są używane operacyjnie po stronie deploy/monitoringu (CI + SRE), a nie przez polling UI.
