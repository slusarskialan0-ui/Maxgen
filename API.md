...
| POST | `/leads/generate-offline` | Generator leadów offline (reklamy/social/marketplace/formularze) |
...
Wyzwalacze: `new_lead`, `stage_change`, `score_above`, `score_below`, `follow_up_due`  
Akcje: `assign`, `follow_up`, `close`, `offer`, `predict`, `notify`

### Oferty (`/offers`) — NOWE
| Metoda | URL | Opis |
|--------|-----|------|
| GET | `/offers` | Lista ofert |
| POST | `/offers` | Utwórz ofertę |
| GET | `/offers/{id}` | Szczegóły oferty |
| PATCH | `/offers/{id}` | Aktualizuj status/kwotę/termin |
| DELETE | `/offers/{id}` | Usuń ofertę |

### Płatności (`/payments`) — NOWE
| Metoda | URL | Opis |
|--------|-----|------|
| GET | `/payments` | Lista płatności offline |
| POST | `/payments` | Utwórz płatność |
| PATCH | `/payments/{id}` | Zmień status/notatkę |
| POST | `/payments/{id}/confirm` | Potwierdź płatność i wygeneruj kod |
| GET | `/payments/logs/recent` | Ostatnie logi płatności |
