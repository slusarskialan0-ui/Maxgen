...
export default function SetupWizard() {
  ...
  const handleCopyHealth = () => {
    ...
    navigator.clipboard.writeText(url)
      .then(() => setKeepAliveMsg(`✅ Skopiowano: ${url}`))
      .catch(() => setKeepAliveMsg('❌ Nie udało się skopiować URL'))
  }
      ...
      {tab === 'keepalive' && (
        <div className="space-y-4">
          <div className="rounded-xl border bg-amber-50 p-4 text-sm text-amber-800 space-y-3">
            ...
            {keepAliveMsg && (
              <div className={`text-xs ${keepAliveMsg.startsWith('✅') ? 'text-green-700' : 'text-red-700'}`}>
                {keepAliveMsg}
              </div>
            )}
