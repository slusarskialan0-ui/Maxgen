...
export default function AutoContact() {
  ...
  const sendContact = async () => {
    ...
    try {
      ...
      setStatusMessage('✅ Status klienta zaktualizowany pomyślnie.')
      setSelected(null)
      setMessage('')
    } catch (error) {
      setStatusMessage(`❌ ${error.response?.data?.detail || 'Nie udało się zaktualizować statusu klienta.'}`)
    } finally {
      setSending(false)
    }
  }
    ...
    <div className="p-4 md:p-8 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
              ...
              <>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-2.5 rounded-lg text-white font-semibold text-sm transition-colors ${selected.email ? 'bg-blue-600 hover:bg-blue-700' : 'bg-gray-300 cursor-not-allowed'} disabled:opacity-60`}
                  >
                    {sending ? '⏳ Przetwarzanie...' : selected.email ? '📧 Oznacz jako skontaktowany' : '⚠ Brak adresu email'}
                  </button>
                  <button
          ...
          {statusMessage && (
            <div className={`mt-4 rounded-xl border p-3 text-sm ${statusMessage.startsWith('✅') ? 'border-green-200 bg-green-50 text-green-700' : 'border-red-200 bg-red-50 text-red-700'}`}>
              {statusMessage}
            </div>
          )}
        </div>
