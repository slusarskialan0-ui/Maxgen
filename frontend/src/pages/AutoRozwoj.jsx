...
export default function AutoRozwoj() {
  ...
  const runAction = async (action, endpoint, body) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.detail || data.message || 'Błąd żądania')
      setActionMessage(`✅ ${data.message || 'Akcja zakończona pomyślnie.'}`)
      if (endpoint === '/api/auto-scale') {
        api.get('/system/forecast').then((res) => setForecast(res.data)).catch(() => {})
      }
    } catch (error) {
      setActionMessage(`❌ ${error.message || 'Nie udało się wykonać akcji.'}`)
    } finally {
      setBusyAction('')
    }
  }
    ...
    <div className="p-8 space-y-6">
      ...
      <div className="rounded-2xl border bg-gradient-to-r from-indigo-600 to-blue-600 p-6 text-white">
        ...
        <div className="mt-4">
          <input
            value={socialToken}
            onChange={(event) => setSocialToken(event.target.value)}
            placeholder="Token social media (opcjonalnie)"
            className="w-full rounded-xl border border-white/30 bg-white/10 px-3 py-2 text-sm text-white placeholder:text-blue-100 focus:outline-none focus:ring-2 focus:ring-white/40"
          />
        </div>
        <div className="mt-5 grid grid-cols-1 gap-2 md:grid-cols-2">
          <button
            onClick={() => runAction('scale', '/api/auto-scale')}
            disabled={busyAction !== ''}
            className="rounded-xl bg-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {busyAction === 'scale' ? 'Przetwarzanie...' : '⚙️ Auto-Rozwój: Skaluj zasoby'}
          </button>
          <button
            onClick={() => runAction('scraper', '/api/run-scraper')}
            disabled={busyAction !== ''}
            className="rounded-xl bg-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {busyAction === 'scraper' ? 'Przetwarzanie...' : '🕸️ Uruchom skraper'}
          </button>
          <button
            onClick={() => runAction('niche', '/api/niche-action')}
            disabled={busyAction !== ''}
            className="rounded-xl bg-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {busyAction === 'niche' ? 'Przetwarzanie...' : '🎯 Akcja niszowa'}
          </button>
          <button
            onClick={() => runAction('social', '/api/social/connect', { platform: 'facebook', token: socialToken })}
            disabled={busyAction !== ''}
            className="rounded-xl bg-white/15 px-4 py-2 text-sm font-semibold hover:bg-white/20 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {busyAction === 'social' ? 'Przetwarzanie...' : '🔗 Połącz social media'}
          </button>
        </div>
        {actionMessage && (
          <div className="mt-3 rounded-xl bg-white/15 px-3 py-2 text-sm text-white">{actionMessage}</div>
        )}
      </div>
