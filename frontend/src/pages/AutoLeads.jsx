...
import { stateApi } from '../api/config'
...
export default function AutoLeads() {
  ...
  const [nicheStatus, setNicheStatus] = useState(null)
  ...
  useEffect(() => {
    const bootstrap = async () => {
      const [voivRes, industriesRes, stateRes] = await Promise.all([
        api.get('/voivodeships'),
        api.get('/industries'),
        stateApi.getData('auto_leads_state').catch(() => null),
      ])
      setVoivodeships(voivRes.data)
      setIndustries(industriesRes.data)

      const defaults = industriesRes.data.map(i => i.name)
      const saved = stateRes?.data?.value
      if (saved?.selectedVoivodeship) {
        setSelectedVoiv(saved.selectedVoivodeship)
      }
      if (Array.isArray(saved?.selectedIndustries) && saved.selectedIndustries.length > 0) {
        setSelectedIndustries(saved.selectedIndustries)
      } else {
        setSelectedIndustries(defaults)
      }
    }
    bootstrap().catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    stateApi.getNicheStatus(selectedVoiv)
      .then((res) => setNicheStatus(res.data?.data || null))
  ...
  const startPipeline = async () => {
    ...
    try {
      await stateApi.save('auto_leads_state', {
        selectedVoivodeship: selectedVoiv,
        selectedIndustries,
      })
      await stateApi.save(`niche_status:${selectedVoiv}`, runningState)
      setNicheStatus(runningState)
      await api.post('/pipeline/run', { voivodeship: selectedVoiv, industries: selectedIndustries })
    } catch (err) {
      const failedState = {
        status: 'nie_rozpoczete',
        updated_at: new Date().toISOString(),
        error: err.response?.data?.detail || err.message,
      }
      try {
        await stateApi.save(`niche_status:${selectedVoiv}`, failedState)
        setNicheStatus(failedState)
      } catch {
        // ignore cloud sync fallback errors
      }
      setRunning(false)
      setErrorMsg(`Błąd zapisu do chmury: ${err.response?.data?.detail || err.message}`)
      return
    }
    // Poll for status
    pollRef.current = setInterval(async () => {
      ...
      if (r.data.status === 'done') {
        ...
        try {
          await stateApi.save(`niche_status:${selectedVoiv}`, {
            status: 'zakonczone',
            updated_at: new Date().toISOString(),
            result: r.data.result,
          })
          const nicheRes = await stateApi.getNicheStatus(selectedVoiv)
          setNicheStatus(nicheRes.data?.data || null)
        } catch {
          // ignore cloud niche status sync errors
        }
        // Fetch logs
      ...
      <div className="bg-white rounded-xl border p-6 mb-6">
        ...
        {nicheStatus?.status && (
          <div className="mt-3 text-xs text-gray-500">
            Status niszy w chmurze: <span className="font-semibold">{nicheStatus.status}</span>
          </div>
        )}
      </div>
